 ...
 */
app.controller('indexCtrl',function($scope,$state,$cookieStore,$http,USER_ROLES,AuthService,Session,$messager,StringUtile){
    ...
    $scope.selectUser=function(user){
        $scope.to=user.name;
        socket.emit('new message',username,$scope.to, '');
    }
    ...
    function sendMessage () {
        ...
        if (message && connected) {
            ...
            socket.emit('new message',username,$scope.to, message);
        }
    }
    ...
    function addChatMessage (data, options) {
        ...
        var $usernameDiv = $('<span class="username label-info" style="margin-right: 20px"/>')
            .text(data.username+"  ")
            .css('color', 'white');
    ...
    function resChatMessage (data, options) {
        // Don't fade the message in if there is an 'X was typing'
        var $typingMessages = getTypingMessages(data);
        options = options || {};
        if ($typingMessages.length !== 0) {
        ...
        var $usernameDiv = $('<span class="username label-success " style="margin-left: 20px"/>')
            .text("  "+data.username)
            .css('color', 'white');
        var $messageBodyDiv = $('<span class="messageBody">')
            .text(data.message);
    ...
    $window.keydown(function (event) {
        ...
        if (event.which === 13) {
            if (username) {
                ...
                sendMessage();
                socket.on('new message'+$scope.to, function (data) {
                    addChatMessage(data);
                });
                socket.emit('stop typing');
                typing = false;
